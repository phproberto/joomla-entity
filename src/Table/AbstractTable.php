...
abstract class AbstractTable extends Table implements TableInterface
{
	...
	/**
	 * Array with alias for "special" columns such as ordering, hits etc.
	 *
	 * @var    array
	 */
	...
	protected function doDelete($pk = null)
	{
		...
		else
		{
			if (is_array($pk))
			{
				...
				$pk = array_map([$this->{'_db'}, 'quote'], $pk);
				$pk = implode(',', $pk);
			}
	...
	public function doPublish($pks = null, $state = 1, $userId = 0)
	{
		...
		foreach ($pks as $pk)
		{
			...
			{
				$query->where('
					(' . $this->getColumnAlias('checked_out') . ' = 0'
